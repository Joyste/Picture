package com.test.picture.helper;
 ...
 */
public class Camera2ActivityHelper {
    private static final String TAG = Camera2ActivityHelper.class.getSimpleName();
    ...
    private static final int PREVIEW_HEIGHT = 2016;                             //预览的高度
    private static final int SAVE_WIDTH = 2160;                                  //保存图片的宽度
    private static final int SAVE_HEIGHT = 4032;                                //保存图片的高度

    private Camera2Activity mActivity;
    private TextureView mTextureView;

    private int screenWidth;

    public Camera2ActivityHelper(Camera2Activity activity, TextureView textureView) {
...
//            mActivity.toast("相机硬件不支持新特性")
            ...
            mImageReader.setOnImageAvailableListener(new ImageReader.OnImageAvailableListener() {
                ...
                public void onImageAvailable(ImageReader reader) {
                    ...
                    bitmap = BitmapUtils.scaleWithWH(bitmap, SAVE_WIDTH, SAVE_WIDTH * bitmap.getHeight() / bitmap.getWidth());
...
//            captureRequestBuilder.set(CaptureRequest.JPEG_ORIENTATION, mCameraSensorOrientation);      //根据摄像头方向对保存的照片进行旋转，使其为"自然方向"
    ...
    private Size getBestSize(int targetWidth, int targetHeight, int maxWidth, int maxHeight, List<Size> sizeList) {
        ...
        Size size;
        if (bigEnough.size() > 0) {
            size = Collections.max(bigEnough, new CompareSizesByArea());
...
//        mDisplayRotation = mActivity.getWindowManager().getDefaultDisplay().getRotation();
     ...
     * 销毁会话
     */
    public void releaseCameraCaptureSession() {
